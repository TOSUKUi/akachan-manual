---
band: m10-12
label: 10〜12か月
monthsFrom: 10
monthsTo: 12
intro: つかまり立ちや伝い歩きが始まり、1歳前後で離乳食は卒乳の時期も視野に入ります。1歳をまたぐと麻しん・風しん（MR）1期など定期予防接種の追加が続くので、予約の順番を先に決めるこの時期からが実務の山場です。
caution: 1歳からは麻しん・風しん（MR）1期など定期予防接種が続きます。予診票がない場合は区の電子申請で交付申請が可能です。転勤で品川区へ引っ越す場合は、転入届（転出届受理から14日以内）とは別に母子健康手帳の引き継ぎと予防接種の予診票発行が必要で、品川区で受ける予防接種は予約制です。前の自治体で受けた健診の受診票も窓口で交付してもらえます。
sources:
  - name: こどもの予防接種（品川区）
    url: https://www.city.shinagawa.tokyo.jp/PC/kodomo/kodomo-ninnshinn/kodomo-ninnshinn-yobousessyu/hpg000033448.html
    checked: "2026-09-02"
  - name: 品川区 転入届（別の自治体から品川区に引っ越したとき）
    url: https://www.city.shinagawa.tokyo.jp/PC/procedure/procedure-zyuumin_inkan/procedure-zyuumin_inkan-zyuumin/procedure-zyuumin_inkan-zyuumin-todoke/tennyu20241020.html
    checked: "2026-09-02"
  - name: 品川区 パパの子育てスタートブック
    url: https://www.city.shinagawa.tokyo.jp/PC/kodomo/kodomo-ninnshinn/hpg000016964.html
    checked: "2026-09-02"
  - name: 品川区 子どもすこやか医療費助成
    url: https://www.city.shinagawa.tokyo.jp/PC/kodomo/kodomo-iryohizyosei/hpg000017744.html
    checked: "2026-09-02"
  - name: ピジョン はじめての育児 生後8か月の発達・成長
    url: https://pigeon.info/baby/getsurei/getsurei-8.html
    checked: "2026-09-02"
  - name: Amazon.co.jp 検索「型はめ おもちゃ」
    url: https://www.amazon.co.jp/s?k=%E5%9E%8B%E3%81%AF%E3%82%81%20%E3%81%8A%E3%82%82%E3%81%A1%E3%82%83
    checked: "2026-09-01"
  - name: Amazon.co.jp 検索「ベビー おさかなつり」
    url: https://www.amazon.co.jp/s?k=%E3%83%99%E3%83%93%E3%83%BC%20%E3%81%8A%E3%81%95%E3%81%8B%E3%81%AA%E3%81%A4%E3%82%8A
    checked: "2026-09-01"
  - name: 環境省 WBGT 公開ページ（熱中症予防情報）
    url: https://www.wbgt.env.go.jp/
    checked: "2026-09-02"
  - name: Amazon.co.jp 検索「キッズ帽子 紫外線対策」
    url: https://www.amazon.co.jp/s?k=%E3%82%AD%E3%83%83%E3%82%BA%E5%B8%BD%E5%AD%90%20%E7%B4%AB%E5%A4%96%E7%B7%9A%E5%AF%BE%E7%AD%96
    checked: "2026-09-01"
  - name: アカチャンホンポ マグ・ストローマグ 商品一覧
    url: https://shop.akachan.jp/shop/c/cb150/
    checked: "2026-09-02"
  - name: アカチャンホンポ コップ・食器 商品一覧
    url: https://shop.akachan.jp/shop/c/cb210/
    checked: "2026-09-02"
  - name: アカチャンホンポ ファーストシューズ 商品一覧
    url: https://shop.akachan.jp/shop/c/cb086/
    checked: "2026-09-02"
  - name: 品川区 一時保育
    url: https://www.city.shinagawa.tokyo.jp/PC/kodomo/kodomo-hoyou/kodomo-hoyou-ichizi/hpg000033215.html
    checked: "2026-09-02"
  - name: ピジョン はじめての育児 1歳の発達・成長
    url: https://pigeon.info/baby/getsurei/getsurei-12.html
    checked: "2026-09-02"
  - name: 東京都福祉局 災害時に備えて 知っていますか？乳児用液体ミルク
    url: https://www.fukushi.metro.tokyo.lg.jp/kodomo/koho/nyujiyoekitaimilk
    checked: "2026-09-02"
  - name: 品川区 区民避難所の機能充実・運営組織
    url: https://www.city.shinagawa.tokyo.jp/PC/bosai/bosai2/jishin/hinanjyo/hpg000007756.html
    checked: "2026-09-02"
  - name: 西松屋オンラインストア 液体ミルク 商品検索
    url: https://www.24028-net.jp/item_list.html?searchbox=1&q=%E6%B6%B2%E4%BD%93%E3%83%9F%E3%83%AB%E3%82%AF
    checked: "2026-09-02"
  - name: 西松屋 はじめてのベビーウェア（サイズ表・参考月齢つき）
    url: https://www.24028.jp/premama/guide/babywear/
    checked: "2026-09-02"
  - name: 西松屋オンラインストア 80cmサイズ商品一覧
    url: https://www.24028-net.jp/category/S4/
    checked: "2026-09-02"
  - name: 西松屋 育児グッズの準備品リスト（ベビーカーA型/B型）
    url: https://www.24028.jp/premama/preparation-item/childcare/
    checked: "2026-09-02"
  - name: 西松屋オンラインストア ベビーカー 商品一覧
    url: https://www.24028-net.jp/category/TRIP_STROLLER/
    checked: "2026-09-02"
  - name: アカチャンホンポ セカンドベビーカー・B型ベビーカー 商品一覧
    url: https://shop.akachan.jp/shop/c/cb283/
    checked: "2026-09-02"
support:
  - id: ichiji-prep
    title: 一時保育の事前登録と料金
    detail: 区立園の一時保育は事前登録・事前予約制。生後満4か月〜就学前で、利用料は1日2,000円（各園2人まで、午前7時30分〜午後6時30分）。
    source: https://www.city.shinagawa.tokyo.jp/PC/kodomo/kodomo-hoyou/kodomo-hoyou-ichizi/hpg000033215.html
  - id: yobou-1sai
    title: 1歳からの予防接種（MR1期・水痘・小児用肺炎球菌追加）
    detail: MR（麻しん・風しん）1期は1歳から2歳の誕生日の前日までに1回、水痘は1歳から3歳の誕生日の前日までに2回、小児用肺炎球菌の追加は初回完了後60日以上あけて生後12〜15か月が目安です。
    source: https://www.city.shinagawa.tokyo.jp/PC/kodomo/kodomo-ninnshinn/kodomo-ninnshinn-yobousessyu/hpg000033448.html
  - id: shinagawa-tennyu
    title: 品川区に転入したら（転入届と子育ての引継ぎ）
    detail: 転入届は転出届受理から14日以内。あわせて世帯の状況に応じて国民健康保険・児童手当・保育所入所の申請が必要です。子ども医療証は保険証と引き換えに窓口で即日交付されます。
    source: https://www.city.shinagawa.tokyo.jp/PC/procedure/procedure-zyuumin_inkan/procedure-zyuumin_inkan-zyuumin/procedure-zyuumin_inkan-zyuumin-todoke/tennyu20241020.html
  - id: papa-book
    title: パパの子育てスタートブック（東京都）
    detail: 母体の心身の変化と子どもの成長、その時々の父親としての役割や子育ての知識をまとめた冊子。区の子育てページから読めます。
    source: https://www.city.shinagawa.tokyo.jp/PC/kodomo/kodomo-ninnshinn/hpg000016964.html
  - id: medical-fee
    title: 子どもすこやか医療費助成
    detail: 0〜18歳（高校3年生相当の年齢まで）が対象で、区内の医療機関で保険証といっしょに医療証を提示すると窓口の自己負担が助成されます。健康診断・予防接種・薬の容器代などは対象外です。
    source: https://www.city.shinagawa.tokyo.jp/PC/kodomo/kodomo-iryohizyosei/hpg000017744.html
items:
  - id: m1012-shape-toy
    name: 型はめ・積み木（指先を使うおもちゃ）
    category: asobi
    need: must
    startMonth: 10
    endMonth: 30
    size: 口に入らない大きさ
    note: 手の発達が早く、親が付き添わなくても自分で遊べる時間の第一歩になるため、自分で組み替えて遊べるおもちゃはこの時期から長く使えます。部品が口に入らないサイズか、洗える素材かを基準に選びます（目安 1セット）
    whySources:
      - https://pigeon.info/baby/getsurei/getsurei-8.html
    price:
      low: 498
      high: 3980
      unit: 1セット
      checked: "2026-09-02"
      sources:
        - https://www.amazon.co.jp/s?k=%E5%9E%8B%E3%81%AF%E3%82%81%20%E3%81%8A%E3%82%82%E3%81%A1%E3%82%83
    shops:
      - kind: nishimatyaya
        q: 型はめ・積み木
      - kind: akachan
        q: 型はめ・積み木
      - kind: amazon
        q: 型はめ・積み木
  - id: m1012-fishing-toy
    name: おさかなつりなど指先遊びのおもちゃ
    category: asobi
    need: useful
    startMonth: 10
    endMonth: 24
    size: 部品が大きく丸みがあるもの
    note: 指先を使う遊びは言葉や集中力の土台になり、寝る前以外の時間つぶしにも使えるため、親子の遊び方を見本で示してあげると続きやすいです。音の出るものは時間帯を決めておくと夜泣きの妨げになりません（目安 1個）
    whySources:
      - https://pigeon.info/baby/getsurei/getsurei-8.html
    price:
      low: 660
      high: 2699
      unit: 1個
      checked: "2026-09-02"
      sources:
        - https://www.amazon.co.jp/s?k=%E3%83%99%E3%83%93%E3%83%BC%20%E3%81%8A%E3%81%95%E3%81%8B%E3%81%AA%E3%81%A4%E3%82%8A
    shops:
      - kind: nishimatyaya
        q: おさかなつりなど指先遊びのおもちゃ
      - kind: akachan
        q: おさかなつりなど指先遊びのおもちゃ
      - kind: amazon
        q: おさかなつりなど指先遊びのおもちゃ
  - id: m1012-milk-stock
    name: 液体ミルク（非常用備蓄）
    category: arau
    need: must
    startMonth: 10
    endMonth: 24
    size: 240ml缶を6〜12本
    note: 調乳不要でそのまま飲めるため、備蓄用には必須に近い保険です。東京都は「大規模災害では電気・ガス・水道の復旧に1週間以上かかる恐れがあり、粉ミルク利用世帯には液体ミルクが有用」と案内。品川区も備蓄倉庫に液体ミルクやベビーフードを備蓄していますが、届くまでを自力でしのぐには自宅備蓄が必要です。西松屋の明治「ほほえみ／ステップ らくらくミルク」240ml×30缶は税込6,408〜9,324円で、1缶あたり約213〜311円でした（目安 6〜12本）
    whySources:
      - https://www.fukushi.metro.tokyo.lg.jp/kodomo/koho/nyujiyoekitaimilk
      - https://www.city.shinagawa.tokyo.jp/PC/bosai/bosai2/jishin/hinanjyo/hpg000007756.html
    price:
      low: 213
      high: 311
      unit: 1缶（240ml）
      checked: "2026-09-02"
      sources:
        - https://www.24028-net.jp/item_list.html?searchbox=1&q=%E6%B6%B2%E4%BD%93%E3%83%9F%E3%83%AB%E3%82%AF
    shops:
      - kind: nishimatyaya
        q: 液体ミルク
      - kind: akachan
        q: 液体ミルク
      - kind: amazon
        q: 液体ミルク
  - id: m1012-hat
    name: 帽子（紫外線対策・防寒）
    category: kiru
    need: must
    startMonth: 10
    endMonth: 36
    size: 頭囲を実測して選ぶ
    note: 公園での時間が増え、紫外線と温度調整を親が管理しなければならないため、外出時間が増える分、帽子の不足がそのまま日焼け対策の漏れになります。洗濯を考慮して2枚持っておくと安心です（目安 1〜2個）
    whySources:
      - https://www.wbgt.env.go.jp/
    price:
      low: 699
      high: 1789
      unit: 1個
      checked: "2026-09-02"
      sources:
        - https://www.amazon.co.jp/s?k=%E3%82%AD%E3%83%83%E3%82%BA%E5%B8%BD%E5%AD%90%20%E7%B4%AB%E5%A4%96%E7%B7%9A%E5%AF%BE%E7%AD%96
    shops:
      - kind: nishimatyaya
        q: 帽子
      - kind: akachan
        q: 帽子
      - kind: uniqlo
        q: 帽子
  - id: m1012-cup
    name: ストローマグ・コップ
    category: tabe
    need: must
    startMonth: 9
    endMonth: 36
    size: こぼしにくい構造1個＋洗い替え1個
    note: 母乳・ミルクから食事や飲み物へ移行する時期のため、自分で持って飲めるコップやマグの練習期間が必要です。月齢に合わせてストローからふつうのコップへ切り替えるので、最初から高価なものは避けると無駄が減ります。
    whySources:
      - https://pigeon.info/baby/getsurei/getsurei-12.html
      - https://shop.akachan.jp/shop/c/cb150/
    price:
      low: 418
      high: 2640
      unit: 1個（税込）
      checked: "2026-09-02"
      sources:
        - https://shop.akachan.jp/shop/c/cb150/
        - https://shop.akachan.jp/shop/c/cb210/
    shops:
      - kind: nishimatyaya
        q: ストローマグ
      - kind: akachan
        q: マグ
  - id: m1012-first-shoes
    name: ファーストシューズ
    category: kiru
    need: useful
    startMonth: 10
    endMonth: 15
    size: 実測より0.5〜1.0cm大きいサイズ1足
    note: つかまり立ちや伝い歩きで足を床につく時間が増える時期のため、外で歩かせる場面で足の裏を守る靴が要ります。室内では裸足で十分なので、外出用の1足をサイズを実測してから選ぶと買い直しを防げます。
    whySources:
      - https://pigeon.info/baby/getsurei/getsurei-12.html
      - https://shop.akachan.jp/shop/c/cb086/
    price:
      low: 1958
      high: 7700
      unit: 1足（税込）
      checked: "2026-09-02"
      sources:
        - https://shop.akachan.jp/shop/c/cb086/
    shops:
      - kind: nishimatyaya
        q: ファーストシューズ
      - kind: akachan
        q: ファーストシューズ
  - id: m1012-size80
    name: 服の買い足し（80サイズ）
    category: kiru
    need: useful
    startMonth: 10
    endMonth: 16
    size: 80cm／西松屋サイズ表の参考月齢は12か月前後
    note: 立っち・よちよち歩きでズボンの股上と膝、よだれかけの消耗が伸びます。上下を1〜2サイズ上げる時期ですが、冬物まで一気には揃えず、外出時間と重ね着でしのげる分だけ先に揃えるほうが無駄がありません（目安 2〜3点）
    whySources:
      - https://www.24028.jp/premama/guide/babywear/
    price:
      low: 328
      high: 977
      unit: 1点（税込）
      checked: "2026-09-02"
      sources:
        - https://www.24028-net.jp/category/S4/
    shops:
      - kind: nishimatyaya
        q: 80cm 服
      - kind: akachan
        q: ベビー服 80cm
      - kind: uniqlo
        q: ベビー 80cm
  - id: m1012-b-stroller
    name: B型ベビーカー（セカンドベビーカー）の検討
    category: ugoku
    need: useful
    startMonth: 9
    endMonth: 36
    size: 1台（A型の継続使用・譲渡と合わせて判断）
    note: 西松屋の準備品リストは「生後1か月から使えるA型」と「おすわりができるようになる7か月頃から使えるB型（バギータイプを含む）」を分けて案内しています。A型は5〜6kg級、B型は3〜4kg級で、階段押し上げ・電車移動・車のトランクの負担が消えます。自宅の階段数と今後の外出頻度を数えてから買えば、買わずに済んだ場合の無駄を避けられます（目安 0〜1台）
    whySources:
      - https://www.24028.jp/premama/preparation-item/childcare/
    price:
      low: 21998
      high: 69960
      unit: 1台（税込・新品）
      checked: "2026-09-02"
      sources:
        - https://www.24028-net.jp/category/TRIP_STROLLER/
        - https://shop.akachan.jp/shop/c/cb283/
    shops:
      - kind: nishimatyaya
        q: B型 ベビーカー
      - kind: akachan
        q: B型ベビーカー
      - kind: amazon
        q: B型 ベビーカー
---
